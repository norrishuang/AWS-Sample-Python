...
def post_run_event(datahub_endpoint_url, run_event, parsed_run_event):
    ...
    try:
        # Data Hub API 端点
        data_hub_url = f"{datahub_endpoint_url}/openapi/openlineage/api/v1/lineage"
        ...
        if response.status_code == 200:
            print("OpenLineage send to Data Hub Success.")
        else:
            print(f"Send failure, Status Code: {response.status_code}")
            print(response.text)
        # print("  Succeeded.")
...
# precision for the startTime parameter to the CloudWatch filter_log_events() API.
