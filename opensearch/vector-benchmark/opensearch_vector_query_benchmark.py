...
def build_exact_knn_index(vectors, space_type='innerproduct'):
    ...
    Args:
        ...
        space_type: Space type used in OpenSearch ('innerproduct', 'l2', etc.)
...
def worker_process(args):
    ...
    host, port, user, password, index_name, vector_dimension, k, worker_id, stop_flag, result_queue, evaluate_recall, all_doc_ids, all_vectors, exact_knn, random_queries, space_type, debug = args
    ...
    while not stop_flag.value:
        ...
        if evaluate_recall and all_vectors is not None and len(all_vectors) > 0 and not random_queries:
            ...
            if debug:
                print(f"Worker {worker_id}: Using sample vector from document {query_doc_id}")
        else:
            # Generate random vector for performance testing
            vector = np.random.uniform(-1, 1, vector_dimension).tolist()
            query_doc_id = None
            if debug and random_queries:
                print(f"Worker {worker_id}: Using random vector for query")
        ...
        try:
            ...
            if evaluate_recall and exact_knn is not None:
                ...
                if debug:
                    print(f"Worker {worker_id}: OpenSearch returned {len(opensearch_ids)} results")
                ...
                if debug:
                    print(f"Worker {worker_id}: Query {query_id} recall: {recall}")
...
def run_benchmark(host, port, user, password, index_name, vector_dimension, k, concurrency, duration_seconds, evaluate_recall=False, recall_sample_size=None, random_queries=False, aws_auth=False, region=None, space_type='innerproduct'):
    ...
    if evaluate_recall:
        try:
            ...
            if len(all_vectors) < k + 1:
                ...
                # Build exact KNN index with the specified space type
                exact_knn = build_exact_knn_index(all_vectors, space_type)
                
                print(f"Recall evaluation prepared with {len(all_doc_ids)} documents")
                print(f"Using space type: {space_type} for recall evaluation")
    ...
    for i in range(concurrency):
        args = (host, port, user, password, index_name, vector_dimension, k, i, stop_flag, result_queue, evaluate_recall, all_doc_ids, all_vectors, exact_knn, random_queries, space_type, False)  # Last param is debug flag
...
def main():
    ...
    parser.add_argument('--space-type', type=str, default='innerproduct',
                        help='Space type used in OpenSearch index (innerproduct, l2, etc.). Default: innerproduct')
    ...
    try:
        ...
        # Fetch index mapping to determine space type if not specified
        if not args.space_type:
            index_config = fetch_index_mapping(client, args.index)
            args.space_type = index_config['space_type']
        
        print(f"Using space type '{args.space_type}' for vector similarity calculations")
